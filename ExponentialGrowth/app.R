...
# Pre-Shiny stuff
makeDF <- function(r, n0=1, t_intervene=50){
  df <- tibble(Time = 0:50,
               N = n0*exp(r*Time),
               N_intervention =
                 n0*exp(r*t_intervene) +
                 ifelse(Time < t_intervene,
                        n0*exp(r*t_intervene)*(exp(r*(Time-t_intervene))-1),
                        n0*exp(r*t_intervene)*(exp((r/2)*(Time-t_intervene))-1)
                 )
  )
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      sliderInput("n0",
                  "\\(log_{10}\\) Initial population size",
                  min = 0, max = 5,
                  value = 0,
                  step = 1
      ),

      p("\\(N(0) = \\)", textOutput("N0", inline = TRUE)),
    ...
    mainPanel(
      tabsetPanel(type = "tabs",
                  ...
                  tabPanel("Exponential growth",
                           p("Try setting \\(N_0 = 1\\) and adjusting \\(r\\) so that the line hits the point."),
                           plotOutput("expPlot"),
                  ...
                  tabPanel("Ebola outbreaks",
                           ...
                           p("This panel fits a linear regression to the data up to the intervention start date (value in slider, below) and then projects that line forward. (The sliders to the left have been turned off so the regression can be fit.)"),
                           p("See if you can guess when that intervention started. (Note that unlike our simple example in the previous tab, interventions in the real world do not happen instantaneously everywhere, so it may not look sudden.)"),
